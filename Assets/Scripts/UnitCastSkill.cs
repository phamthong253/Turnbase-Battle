...
public class UnitCastSkill : MonoBehaviour
{
    ...
    private bool CanUseSkill(SkillSO skill)
    {
        ...
        // Kiểm tra Mana
        if (_unitController.currentMana < skill.manaCost)
        {
            Debug.Log($"[UnitCastSkill] Không đủ mana để dùng {skill.skillName}. Cần {skill.manaCost}, có {_unitController.currentMana}");
            return false;
        }
